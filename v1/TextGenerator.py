...
from prompts import system_prompt4demand_gen, system_prompt4code_and_images_meta_gen
from structured_output import GameDemand, GameCodeAndImagesMeta
...
```json
{
    ...
    "images_meta": dict[string, dict[string, int | string]]  // the metadata of the images used in the game. The key is the name (only image name, without any file name suffix) of the image, the value is the metadata of the image (description, width, height)
}
"""  
        if self.platform == "niya_aws":
            response = self.client.chat.completions.create(
